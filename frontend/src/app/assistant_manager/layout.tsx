...
import {
  LayoutDashboard, ClipboardList, LogOut, Zap, ChevronRight, Trophy, MapPin, Menu, Settings, Key, Calendar
} from 'lucide-react';
...
const navItems = [
  { href: '/assistant_manager/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/assistant_manager/tasks', label: 'Kanban & Tasks', icon: ClipboardList },
  { href: '/assistant_manager/attendance', label: 'Attendance Monitor', icon: MapPin },
  { href: '/assistant_manager/leaderboard', label: 'Leaderboard', icon: Trophy },
  { href: '/assistant_manager/leaves', label: 'Leaves', icon: Calendar },
];

export default function AssistantManagerLayout({ children }: { children: React.ReactNode }) {
  const { user, isLoading, isAssistantManager, logout } = useAuth();
  const router = useRouter();
  const pathname = usePathname();
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [showChangePassword, setShowChangePassword] = useState(false);

  const canAccess = isAssistantManager;
