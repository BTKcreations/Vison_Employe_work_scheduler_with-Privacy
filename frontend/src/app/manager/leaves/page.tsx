...
export default function ManagerLeavesPage() {
  ...
  const handleCancel = async (leaveId: string) => {
    ...
    try { await api.delete(`/leaves/${leaveId}`); await fetchData(); } catch { alert('Failed to cancel leave'); }
  };
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage your team&apos;s leave requests</p>
        </div>
          ...
          return (
            <div key={bal.leave_type} className="glass rounded-xl p-5 border border-slate-200/60 shadow-sm hover:shadow-md transition-all">
              <div className="flex items-center justify-between mb-3">
                <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${config.gradient} flex items-center justify-center`}><Icon className="w-5 h-5 text-white" /></div>
                {bal.allowed !== null && (<span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{percentage}% used</span>)}
              </div>
      ...
      <div className="flex items-center gap-1 mb-6 bg-slate-100/80 rounded-xl p-1 w-fit">
        <button onClick={() => setActiveTab('team')}
          ...
          <Users className="w-3.5 h-3.5 inline mr-1.5" />Team Requests
          {pendingCount > 0 && (<span className="ml-1.5 inline-flex items-center justify-center w-5 h-5 rounded-full bg-rose-500 text-white text-[10px] font-bold">{pendingCount}</span>)}
        </button>
        <button onClick={() => setActiveTab('personal')}
          ...
          <Calendar className="w-3.5 h-3.5 inline mr-1.5" />My Leaves
        </button>
      </div>
      ...
      {activeTab === 'team' && (
        <div className="glass rounded-2xl p-6 border border-slate-200/60 shadow-xl shadow-slate-200/20">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Full team hierarchy</p>
              </div>
            </div>
            <div className="relative">
              <select value={teamFilter} onChange={(e) => { setTeamFilter(e.target.value); setLoading(true); }}
          ...
          {teamLeaves.length === 0 ? (
            <div className="text-center py-16"><Users className="w-12 h-12 text-slate-300 mx-auto mb-3" /><p className="text-sm font-medium text-slate-400">No team leave requests</p></div>
          ) : (
              ...
              {teamLeaves.map((leave) => {
                ...
                return (
                  <div key={leave.id} className="flex items-center gap-4 p-4 rounded-xl border border-slate-100 hover:border-slate-200 hover:bg-slate-50/50 transition-all">
                    <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${typeConf.gradient} flex items-center justify-center flex-shrink-0`}><TypeIcon className="w-5 h-5 text-white" /></div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${statConf.className}`}><StatusIcon className="w-3 h-3" />{statConf.label}</span>
                      </div>
                      ...
                      {leave.reviewer_name && (<p className="text-[10px] text-slate-400 mt-1">Reviewed by <span className="font-semibold">{leave.reviewer_name}</span></p>)}
                    </div>
                    {leave.status === 'pending' && (
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button onClick={() => handleAction(leave.id, 'approved')} disabled={actionLoading === leave.id}
                          className="p-2.5 bg-emerald-50 hover:bg-emerald-100 rounded-xl text-emerald-600 transition-colors" title="Approve">
                        ...
                        <button onClick={() => setRejectModal({ leaveId: leave.id, userName: leave.user_name || 'Unknown' })} disabled={actionLoading === leave.id}
                          className="p-2.5 bg-rose-50 hover:bg-rose-100 rounded-xl text-rose-600 transition-colors" title="Reject">
      ...
      {activeTab === 'personal' && (
        <div className="glass rounded-2xl p-6 border border-slate-200/60 shadow-xl shadow-slate-200/20">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center"><Calendar className="w-5 h-5 text-indigo-500" /></div>
            <div>
          ...
          {myLeaves.length === 0 ? (
            <div className="text-center py-16"><Calendar className="w-12 h-12 text-slate-300 mx-auto mb-3" /><p className="text-sm font-medium text-slate-400">No leave requests yet</p></div>
          ) : (
            <div className="space-y-3">
              {myLeaves.map((leave) => {
                const typeConf = leaveTypeConfig[leave.leave_type];
                ...
                return (
                  <div key={leave.id} className="flex items-center gap-4 p-4 rounded-xl border border-slate-100 hover:border-slate-200 hover:bg-slate-50/50 transition-all group">
                    ...
                    {leave.status === 'pending' && (
                      <button onClick={() => handleCancel(leave.id)} className="opacity-0 group-hover:opacity-100 p-2 hover:bg-rose-50 rounded-lg text-slate-400 hover:text-rose-500 transition-all flex-shrink-0" title="Cancel"><X className="w-4 h-4" /></button>
                    )}
                  </div>
                );
              })}
      ...
      {/* Apply Leave Modal */}
      {showApplyModal && <ApplyLeaveModal onClose={() => setShowApplyModal(false)} onSuccess={() => { setShowApplyModal(false); fetchData(); }} balances={balances} />}

      {/* Reject Modal */}
      {rejectModal && <RejectModal userName={rejectModal.userName} onClose={() => setRejectModal(null)} onReject={(reason) => handleAction(rejectModal.leaveId, 'rejected', reason)} />}
    </div>
  );
}
...
function ApplyLeaveModal({ onClose, onSuccess, balances }: { onClose: () => void; onSuccess: () => void; balances: LeaveBalance[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try { await api.post('/leaves', { leave_type: leaveType, start_date: `${startDate}T00:00:00`, end_date: `${endDate}T00:00:00`, reason: reason.trim() }); onSuccess(); }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6 z-10 border border-slate-200">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-600 to-violet-500 flex items-center justify-center"><Calendar className="w-5 h-5 text-white" /></div>
            <div><h2 className="text-lg font-bold text-slate-800">Apply for Leave</h2><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Submit a new request</p></div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(leaveTypeConfig).map(([key, conf]) => {
                const Icon = conf.icon; const bal = balances.find(b => b.leave_type === key); const isSelected = leaveType === key;
                return (<button key={key} type="button" onClick={() => setLeaveType(key)} className={`flex items-center gap-2.5 p-3 rounded-xl border-2 transition-all text-left ${isSelected ? 'border-indigo-400 bg-indigo-50/50 shadow-sm' : 'border-slate-100 hover:border-slate-200'}`}>
                  ...
                  <div><p className="text-xs font-semibold text-slate-700">{conf.label}</p><p className="text-[10px] text-slate-400">{bal?.remaining !== null ? `${bal?.remaining ?? 0} left` : 'Unlimited'}</p></div>
                </button>);
              })}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs font-bold text-slate-600 uppercase tracking-wider mb-2 block">Start Date</label><input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200" min={new Date().toISOString().split('T')[0]} /></div>
            <div><label className="text-xs font-bold text-slate-600 uppercase tracking-wider mb-2 block">End Date</label><input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200" min={startDate || new Date().toISOString().split('T')[0]} /></div>
          </div>
          {startDate && endDate && new Date(startDate) <= new Date(endDate) && (
            <div className="bg-indigo-50/50 border border-indigo-100 rounded-xl px-4 py-2.5 flex items-center gap-2"><Clock className="w-4 h-4 text-indigo-500" /><span className="text-xs font-semibold text-indigo-700">Duration: {Math.floor((new Date(endDate).getTime() - new Date(startDate).getTime()) / (1000 * 60 * 60 * 24)) + 1} day(s)</span></div>
          )}
          <div><label className="text-xs font-bold text-slate-600 uppercase tracking-wider mb-2 block">Reason</label><textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} placeholder="Describe the reason..." className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 resize-none" maxLength={500} /></div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 z-10 border border-slate-200">
        ...
        <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} placeholder="Provide a reason for rejection (optional)..." className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-200 resize-none mb-4" />
        <div className="flex gap-3">
          ...
          <button onClick={() => onReject(reason)} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-rose-500 to-pink-500 text-white rounded-xl text-sm font-semibold shadow-lg shadow-rose-200 hover:shadow-xl transition-all flex items-center justify-center gap-2"><XCircle className="w-4 h-4" />Reject</button>
        </div>
      </div>
    </div>
  );
