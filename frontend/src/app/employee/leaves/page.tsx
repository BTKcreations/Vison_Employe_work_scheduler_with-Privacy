...
import {
  ...
  Umbrella, Heart, Briefcase, Ban, X, ChevronDown
} from 'lucide-react';
  ...
  const fetchData = useCallback(async () => {
    try {
      const [balRes, leavesRes] = await Promise.all([
        ...
        api.get(`/leaves/me${statusFilter ? `?status=${statusFilter}` : ''}`),
      ]);
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Track your leave balances and requests</p>
        </div>
        <button
            ...
            <div key={bal.leave_type} className="glass rounded-xl p-5 border border-slate-200/60 shadow-sm hover:shadow-md transition-all">
              ...
              <div className="flex items-baseline gap-1.5">
                <span className="text-2xl font-bold text-slate-800">
                  {bal.remaining !== null ? bal.remaining : '∞'}
                </span>
                <span className="text-xs text-slate-400">
                  {bal.allowed !== null ? `/ ${bal.allowed} days` : 'unlimited'}
                </span>
              </div>
              {bal.allowed !== null && (
      ...
      <div className="glass rounded-2xl p-6 border border-slate-200/60 shadow-xl shadow-slate-200/20">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Your requests</p>
            </div>
          </div>
        ...
        {leaves.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-xs text-slate-300 mt-1">Apply for a leave to get started</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {leave.reviewer_name && (
                      <p className="text-[10px] text-slate-400 mt-1">
                        Reviewed by <span className="font-semibold">{leave.reviewer_name}</span>
                        {leave.reviewed_at && ` on ${new Date(leave.reviewed_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}`}
                      </p>
                    )}
                  </div>
                  {leave.status === 'pending' && (
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-2 hover:bg-rose-50 rounded-lg text-slate-400 hover:text-rose-500 transition-all flex-shrink-0"
...
function ApplyLeaveModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const detail = err?.response?.data?.detail || 'Failed to apply for leave';
      setError(detail);
    } finally {
      setSubmitting(false);
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(leaveTypeConfig).map(([key, conf]) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${conf.gradient} flex items-center justify-center flex-shrink-0`}>
                      <Icon className="w-4 h-4 text-white" />
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-slate-700">{conf.label}</p>
                      <p className="text-[10px] text-slate-400">
                        {bal?.remaining !== null ? `${bal?.remaining ?? 0} left` : 'Unlimited'}
                      </p>
                    </div>
                  </button>
                );
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-300"
          ...
          {startDate && endDate && new Date(startDate) <= new Date(endDate) && (
            <div className="bg-indigo-50/50 border border-indigo-100 rounded-xl px-4 py-2.5 flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-indigo-700">
                Duration: {Math.floor((new Date(endDate).getTime() - new Date(startDate).getTime()) / (1000 * 60 * 60 * 24)) + 1} day(s)
              </span>
            </div>
          )}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-300 resize-none"
              maxLength={500}
            />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="flex-1 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-violet-500 text-white rounded-xl text-sm font-semibold shadow-lg shadow-indigo-200 hover:shadow-xl hover:shadow-indigo-300 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {submitting ? (
