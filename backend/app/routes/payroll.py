...
async def create_payroll_draft(
    request: PayrollDraftRequest, hr_user: User = Depends(require_hr_team)
):
...
async def mark_payroll_paid(payroll_id: str, hr_mgr: User = Depends(require_hr_team)):
...
async def manual_recalculate_payroll(
    payroll_id: str, admin: User = Depends(require_hr_team)
):
