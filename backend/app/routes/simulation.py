        ...
        if name == "Sujeeth":
            ...
            for i in range(16):
                ...
                logs.append(
                    Attendance(
                        user_id=user.id,
                        company_id=company.id,
                        check_in=datetime(2026, 5, d, 9, 0),
                        check_out=datetime(2026, 5, d, 18, 0),
                        status="present",
                    )
                )
            for i in range(16, 21):
                ...
                logs.append(
                    Attendance(
                        ...
                        company_id=company.id,
                        check_in=datetime(2026, 5, d, 9, 0),
                        check_out=datetime(2026, 5, d, 21, 0),
                        status="approved_overtime",
                        remarks="Approved Overtime shift",
                    )
                )
        elif name == "Mounika":
            ...
            for i in range(16, 21):
                ...
                logs.append(
                    Attendance(
                        user_id=user.id,
                        company_id=company.id,
                        check_in=datetime(2026, 5, d, 10, 15),
                        check_out=datetime(2026, 5, d, 18, 0),
                        status="late_over_30",
                        remarks="Late check-in > 30 mins",
                    )
                )
        elif name == "Shiva":
            ...
            logs.append(
                Attendance(
                    user_id=user.id,
                    company_id=company.id,
                    check_in=datetime(2026, 5, d, 9, 15),
                    check_out=datetime(2026, 5, d, 18, 0),
                    status="late_under_30",
                    remarks="Late check-in < 30 mins",
                )
            )
            # 2 Overtime shifts
...
async def run_simulation_payroll(admin: User = Depends(require_admin)):
    ...
    for email in emails:
        ...
        if not user:
            raise HTTPException(
                status_code=404,
                detail=f"User {email} not found. Please run seed endpoint first.",
            )

        # Clean existing payroll draft for this month
        existing = await Payroll.find_one(
            Payroll.user_id == user.id, Payroll.month == month_str
        )
        if existing:
        ...
        draft_req = PayrollDraftRequest(
            user_id=str(user.id), month=month_str, automated=True
        )
