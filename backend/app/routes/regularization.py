...
async def request_regularization(
    request: RegularizationApplyRequest, current_user: User = Depends(get_current_user)
):
    ...
    if request.attendance_id:
        ...
        target_date = request.requested_check_in.replace(
            hour=0, minute=0, second=0, microsecond=0
        )
        end_date = request.requested_check_in.replace(
    ...
    return [
        {
            ...
            "requested_check_in": (
                to_utc_iso(r.requested_check_in) if r.requested_check_in else None
            ),
            "requested_check_out": (
                to_utc_iso(r.requested_check_out) if r.requested_check_out else None
            ),
            "reason": r.reason,
...
async def get_pending_regularizations(user: User = Depends(require_management_team)):
    ...
    reqs = (
        await AttendanceRegularization.find(*query_conditions)
        .sort("-created_at")
        .to_list()
    )
...
async def verify_regularization(
    ...
    return {
        "message": "Attendance correction verified successfully, pending manager/admin review.",
        "performed_by": hr_user.role.value,
    }
...
async def review_regularization(
    ...
    return {
        "message": "Attendance correction reviewed successfully, pending final approval.",
        "performed_by": hr_mgr.role.value,
    }
...
async def approve_regularization(
    ...
    if req.status in [RegularizationStatus.APPROVED, RegularizationStatus.REJECTED]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Request already finalized"
        )
    ...
    if not attendance:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Corresponding attendance record not found",
        )
    ...
    attendance.remarks = (
        f"Regularized by {admin.name}. Comments: {action.comments or ''}"
    )
    ...
    if not attendance.location_in:
        try:
            ...
            company = (
                await Company.get(applicant_user.company_id)
                if applicant_user and applicant_user.company_id
                else None
            )
            if (
                company
                and company.office_lat is not None
                and company.office_lng is not None
            ):
            ...
            logger.warning(
                f"Could not set office location on regularized attendance: {loc_err}"
            )
    ...
    try:
        ...
        logger.warning(
            f"Could not automatically recalculate draft payroll on regularization approval: {e}"
        )
    ...
    try:
        payrolls = await Payroll.find(
            Payroll.user_id == req.user_id, Payroll.month == month_str
        ).to_list()
        for p in payrolls:
        ...
        logging.getLogger(__name__).warning(
            f"Could not automatically recalculate payroll on regularization approval: {e}"
        )

    return {
        "message": "Attendance regularization request approved and log successfully updated!",
        "performed_by": admin.role.value,
    }
...
async def reject_regularization(
    ...
    return {
        "message": "Attendance regularization request has been rejected.",
        "performed_by": hr_user.role.value,
    }
