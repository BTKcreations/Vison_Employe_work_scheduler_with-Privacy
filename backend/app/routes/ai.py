...
async def get_task_intelligence(current_user: User = Depends(get_current_user)):
...
async def get_performance_intelligence(current_user: User = Depends(get_current_user)):
...
async def get_attendance_intelligence(current_user: User = Depends(get_current_user)):
...
async def ask_ai_assistant(
    payload: Dict[str, str] = Body(...), current_user: User = Depends(get_current_user)
):
...
async def export_ai_report(
    report_type: str = Query(
        "productivity",
        regex="^(productivity|payroll|team_performance|attendance|executive)$",
    ),
    ...
    if report_type == "payroll" and current_user.role not in [
        UserRole.ADMIN,
        UserRole.HR_MANAGER,
        UserRole.MANAGER,
    ]:
        raise HTTPException(
            status_code=403, detail="Unauthorized role access to payroll reports."
        )
    ...
    if report_format == "excel":
        ...
        header_fill = PatternFill(
            start_color="3F51B5", end_color="3F51B5", fill_type="solid"
        )
        ...
        if report_type == "productivity" or report_type == "executive":
            summary_lines.append(
                f"• Workforce operational productivity index averages {perf_intel['team_average_productivity']}%."
            )
            summary_lines.append(
                f"• Active workload contains {task_intel['total_active_tasks']} tasks under tracking."
            )
            if task_intel["overloaded_employees"]:
                summary_lines.append(
                    f"• Workload capacity warning: {len(task_intel['overloaded_employees'])} employees exceed active thresholds."
                )
        if report_type == "payroll" or report_type == "executive":
            summary_lines.append(
                f"• AI payroll scan flagged {len(payroll_intel.get('alerts', []))} variance anomaly alerts."
            )
        if report_type == "attendance" or report_type == "executive":
            summary_lines.append(
                f"• Late login check-ins flagged {len(attendance_intel.get('late_login_trends', []))} persistent trends in current 30-day window."
            )
        ...
        if alerts_list:
            ws["A12"] = "\n".join(
                [
                    f"⚠ {str(a['details']) if isinstance(a, dict) else str(a)}"
                    for a in alerts_list[:3]
                ]
            )
        else:
            ws["A12"] = (
                "✔ Operational status normal. Zero anomalies flagged in this cycle."
            )
        ws["A12"].font = Font(
            name="Arial", size=10, color="721C24" if alerts_list else "155724"
        )
        ...
        if report_type == "productivity":
            ...
            for p in perf_intel["employee_performance"]:
                ...
                ws.cell(row=curr_row, column=2, value=p["role"].upper()).font = (
                    regular_font
                )
                ws.cell(row=curr_row, column=3, value=p["tasks_assigned"]).font = (
                    regular_font
                )
                ws.cell(row=curr_row, column=4, value=p["tasks_completed"]).font = (
                    regular_font
                )
                ws.cell(row=curr_row, column=5, value=p["productivity_score"]).font = (
                    regular_font
                )
                ws.cell(row=curr_row, column=6, value=p["burnout_risk"]).font = Font(
                    name="Arial",
                    size=10,
                    bold=p["burnout_risk"] == "High",
                    color="D32F2F" if p["burnout_risk"] == "High" else "000000",
                )
            ...
            for c in attendance_intel["consistency_rankings"]:
                ...
                late_warn = next(
                    (
                        l["details"]
                        for l in attendance_intel["late_login_trends"]
                        if l["user_id"] == c["user_id"]
                    ),
                    "Normal",
                )
                abs_warn = next(
                    (
                        a["details"]
                        for a in attendance_intel["absentee_warnings"]
                ...
                ws.cell(row=curr_row, column=2, value=c["total_logs"]).font = (
                    regular_font
                )
                ws.cell(row=curr_row, column=3, value=c["consistency_score"]).font = (
                    regular_font
                )
            ...
            for t in task_intel["task_predictions"][:15]:
                ws.cell(row=curr_row, column=1, value=t["description"]).font = (
                    regular_font
                )
                ws.cell(row=curr_row, column=2, value=t["assigned_to_name"]).font = (
                    regular_font
                )
                ws.cell(row=curr_row, column=3, value=t["priority"].upper()).font = (
                    regular_font
                )
                ws.cell(row=curr_row, column=4, value=t["deadline"]).font = regular_font
                ws.cell(
                    row=curr_row, column=5, value=t["completion_prediction"]
                ).font = Font(
                    name="Arial",
                    size=10,
                    bold=t["completion_prediction"] != "On Time",
                    color=(
                        "D32F2F"
                        if t["completion_prediction"] != "On Time"
                        else "2E7D32"
                    ),
                )
                ws.cell(row=curr_row, column=6, value=t["risk_score"]).font = (
                    regular_font
                )
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=ai_{report_type}_report.xlsx"
            },
        )
        ...
        if alerts_list:
            alerts_li = "".join(
                [
                    f"<li style='color: #D32F2F; margin-bottom: 6px;'>⚠ {str(a['details']) if isinstance(a, dict) else str(a)}</li>"
                    for a in alerts_list[:4]
                ]
            )
        else:
