...
async def get_all_attendance_summary(
    ...
    Optimized with raw PyMongo access, projections, and minimized object creation.
    """
    ...
    if visible_employee_ids is not None:
        ...
        user_query["role"] = {"$in": [r.value if hasattr(r, "value") else r for r in NON_ADMIN_ROLES]}

    if business_unit_id is not None:
        user_query["business_unit_id"] = business_unit_id

    # 1. Fetch employees using raw PyMongo with projection to avoid model instantiation overhead
    user_coll = User.get_pymongo_collection()
    user_projection = {"_id": 1, "name": 1, "email": 1, "reward_points": 1}
    employees = await user_coll.find(user_query, user_projection).to_list(length=100000)
    ...
    # 2. Optimized date handling
    today_start = ist_now().replace(hour=0, minute=0, second=0, microsecond=0)
    five_days_ago = today_start - timedelta(days=4)

    # 3. Fetch attendance logs using raw PyMongo with projection
    ...
    # length=500,000 as per memory directive for attendance logs
    logs = await att_coll.find(att_query, att_projection).to_list(length=500000)

    # 4. Map logs by user_id and date (ISO string) for O(1) lookup
    log_map = {}
    for log in logs:
        uid = str(log["user_id"])
        # Convert check_in to IST date string for stable mapping
    ...
    # Pre-calculate daily metadata to avoid redundant work in the employee loop
    days_meta = []
    for i in range(5):
        day = today_start - timedelta(days=i)
        days_meta.append({
    ...
    days_meta.reverse() # Result should be oldest to newest
    ...
    for emp in employees:
        ...
        for meta in days_meta:
            record = user_logs.get(meta["date_key"])
            entry = {
                "date": meta["iso"],
                "status": "present" if record else "absent",
            }
            if record:
                # Direct dict access is significantly faster than Pydantic model access
                ci = record.get("check_in")
                co = record.get("check_out")
                entry.update({
                    "check_in": to_utc_iso(ci) if ci else None,
                    "check_out": to_utc_iso(co) if co else None,
                    ...
                    "is_regularized": bool(
                        record.get("remarks") and "Regularized" in record["remarks"]
                    )
                })
