...
async def get_all_attendance_summary(
    ...
    Optimized with database-level filtering and raw projections to handle large organizations.
    """
    ...
    if visible_employee_ids is not None:
        ...
        user_query["role"] = {"$in": [role.value for role in NON_ADMIN_ROLES]}

    # Performance: Use raw collection for projection to avoid Beanie overhead
    user_projection = {
        "_id": 1,
        "name": 1,
        "email": 1,
    ...
    # Projection for attendance to keep payload slim
    ...
    # 3. Map logs by user_id and date for O(1) lookup during summary assembly
    log_map: dict = {}
    for log in logs:
        uid = str(log["user_id"])
        # Ensure we use IST for date mapping to match summary logic
    ...
    for emp in employees:
        ...
        for i in range(5):
            ...
            record = log_map.get(uid_str, {}).get(date_str)
            ...
            if record:
                entry.update({
                    ...
                    "is_regularized": bool(record.get("remarks") and "Regularized" in record["remarks"]),
                })
