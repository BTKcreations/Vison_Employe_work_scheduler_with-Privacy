...
async def get_all_attendance_summary(
    ...
    """
    # 1. Build database-level query for employees
    ...
    # Use projection to avoid Beanie/Pydantic overhead for large sets
    employees_cursor = User.get_pymongo_collection().find(
        user_query,
        {"_id": 1, "name": 1, "email": 1, "reward_points": 1}
    )
    # Motor's to_list requires a length argument. Using 10000 as a safe upper bound for a single BU/Tenant dashboard.
    employees = await employees_cursor.to_list(length=10000)

    if not employees:
        return []

    # 2. Pre-calculate date metadata for the 5-day window
    today_start = ist_now().replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    for i in range(5):
        ...
        date_metadata.append({
            "iso_day": to_utc_iso(day),
            "date_key": day.date().isoformat()
        })
    date_metadata.reverse() # Order from 4 days ago to today

    # 3. Fetch attendance records in one pass with projection
    ...
    logs_cursor = Attendance.get_pymongo_collection().find(
        log_query,
        {
    ...
    # 10000 employees * 5 days = 50000 max records
    logs = await logs_cursor.to_list(length=50000)

    # Map logs by user_id and date for O(1) lookup
    log_map: dict = {}
    for log in logs:
        uid = str(log["user_id"])
        # Raw MongoDB dates are UTC datetimes
        dt_ist = log["check_in"].replace(tzinfo=timezone.utc).astimezone(IST)
    ...
    for emp in employees:
        ...
        for meta in date_metadata:
            ...
            if record:
                # Handle null safety for check_out (active sessions)
                check_in = record.get("check_in")
                check_out = record.get("check_out")
                entry.update({
                    "check_in": to_utc_iso(check_in) if check_in else None,
                    "check_out": to_utc_iso(check_out) if check_out else None,
