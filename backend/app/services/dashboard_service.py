...
async def get_all_attendance_summary(
    ...
    """Get last 5 days attendance summary for all employees (or a hierarchy-scoped subset). Optimized with raw collection access."""
    # Build User query
    ...
    # Use raw collection for speed - bypass Beanie/Pydantic model instantiation
    user_collection = User.get_pymongo_collection()
    user_projection = {"_id": 1, "name": 1, "email": 1, "reward_points": 1}
    # Using 100,000 as a safe upper bound for employee list in a single organization.
    employees = await user_collection.find(user_query, user_projection).to_list(length=100000)
    ...
    # Attendance query with projection
    att_query = {
    ...
    att_projection = {
        ...
        "location_in": 1, "location_out": 1, "address_in": 1,
        "address_out": 1, "remarks": 1
    }
    att_collection = Attendance.get_pymongo_collection()
    # 500,000 is a safe bound for 5 days of logs (100k employees * 5 days).
    logs = await att_collection.find(att_query, att_projection).to_list(length=500000)

    # Map logs by user_id and date → store the first record per day
    log_map: dict = {}
    for log in logs:
        uid = str(log["user_id"])
        # Convert check_in to IST date string
        # Assuming check_in is stored as UTC in DB
        dt = log["check_in"].replace(tzinfo=timezone.utc).astimezone(IST)
    ...
    # Pre-calculate dates and their ISO strings for the 5-day window
    days_info = []
    for i in range(5):
        day = today_start - timedelta(days=i)
        days_info.append({
            "date_obj": day,
            "date_iso": to_utc_iso(day),
    ...
    for emp in employees:
        ...
        for info in days_info:
            record = user_logs.get(info["date_str"])
            entry = {
                "date": info["date_iso"],
                "status": "present" if record else "absent",
            }
