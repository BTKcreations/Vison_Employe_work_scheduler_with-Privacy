...
async def get_admin_dashboard(
    ...
    # 1. Get today's attendance user IDs (First step, required for User aggregation)
    ...
    present_user_ids = await Attendance.get_pymongo_collection().distinct("user_id", att_query)
    # Handle the case where Motor returns a coroutine (in some environments like mongomock_motor)
    if asyncio.iscoroutine(present_user_ids):
        present_user_ids = await present_user_ids
    present_user_ids = [PydanticObjectId(uid) for uid in present_user_ids]

    # 2. Optimized User Aggregation: Faceted search for role stats, headcounts and active counts
    ...
    user_pipeline = [
        {"$match": user_match},
        {
            "$facet": {
                "role_stats": [
                    {"$project": {"role": 1, "is_active": 1, "is_present": {"$in": ["$_id", present_user_ids]}}},
                    {
                        "$group": {
                            "_id": "$role",
                            "total": {"$sum": 1},
    ...
    user_data = user_results[0] if user_results else {"role_stats": []}
    ...
    for stat in user_data.get("role_stats", []):
        r_val = stat["_id"]
        role_counts[r_val] = {
            "total": stat["total"],
    ...
    # 3. Optimized Task Aggregation: Single facet for status counts, priority, rewards and performance
    ...
    # Virtual overdue condition: (PENDING or IN_PROGRESS) and DEADLINE < NOW
    # Use string values for Enums in raw aggregation
    active_statuses = [TaskStatus.PENDING.value, TaskStatus.IN_PROGRESS.value]
    completed_statuses = [TaskStatus.COMPLETED.value, TaskStatus.COMPLETED_LATE.value]
                ...
                "status_counts": [
                    {
                        "$group": {
                            "_id": {
                                "$cond": [
                                    {"$and": [{"$in": ["$status", active_statuses]}, {"$lt": ["$deadline", now_utc]}]},
                    ...
                    {"$match": {"deadline": {"$gte": start_date, "$lte": end_date}}},
                    {
                        "$group": {
                            ...
                            "completed_on_time": {
                                "$sum": {
                                    "$cond": [
                                        {"$and": [{"$in": ["$status", completed_statuses]}, {"$lte": ["$completed_at", "$deadline"]}]},
                                        1, 0
                                    ]
                                }
                            },
    ...
    for sc in task_data.get("status_counts", []):
        s_id = sc["_id"]
        if s_id in task_counts:
            task_counts[s_id] = sc["count"]
        task_counts["total"] += sc["count"]

    # Format Priority Distribution
    priority_dist = {"critical": 0, "high": 0, "medium": 0, "regular": 0}
    for pd in task_data.get("priority_dist", []):
        if pd["_id"] in priority_dist:
            priority_dist[pd["_id"]] = pd["count"]

    # Format Performance
    perf_raw = task_data.get("performance", [{}])[0]
    assigned = perf_raw.get("assigned", 0)
    completed = perf_raw.get("completed", 0)
    productivity_pct = round((completed / assigned * 100.0), 1) if assigned > 0 else 0.0
    performance_score = round((perf_raw.get("completed_on_time", 0) / assigned * 100.0), 1) if assigned > 0 else 0.0

    performance_tracking = {
        "assigned_tasks": assigned,
        "completed_tasks": completed,
        "pending_tasks": max(0, assigned - completed - perf_raw.get("overdue", 0)),
        "overdue_tasks": perf_raw.get("overdue", 0),
        "productivity_pct": productivity_pct,
        "performance_score": performance_score,
    }

    # 4. Optimized Recent Activity: single aggregation with lookup
    activity_pipeline = [
        {"$match": {"tenant_id": current_user.tenant_id}},
    ]
    if visible_ids is not None:
        activity_pipeline.append({"$match": {"user_id": {"$in": list(visible_ids)}}})

    activity_pipeline.extend([
        {"$sort": {"timestamp": -1}},
        {"$limit": 10},
        {
            "$lookup": {
    ...
    # Leaderboard (Separate as it's typically highly optimized on its own and requires sorting on user collection)
    if visible_ids is not None:
    ...
    total_rewards = task_data.get("reward_stats", [{}])[0].get("total_rewards", 0) if task_data.get("reward_stats") else 0
