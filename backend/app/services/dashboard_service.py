...
async def get_all_attendance_summary(
    ...
    Optimized with raw PyMongo collection access and pre-calculated date strings.
    """
    ...
    # Bypass Beanie/Pydantic model instantiation for massive speedup
    ...
    # Raw attendance query for efficiency
    att_query = {
    ...
    # Pre-calculate log mapping using O(N) pass
    log_map: dict = {}
    for log in logs:
        uid = str(log["user_id"])
        ...
        date_str = check_in.astimezone(IST).date().isoformat()
    ...
    # Pre-calculate the 5-day window metadata to avoid repeated datetime logic in the employee loop
    days_meta = []
    for i in range(5):
        day = today_start - timedelta(days=i)
        days_meta.append({
