...
async def get_all_attendance_summary(
    ...
    """Get last 5 days attendance summary with database-level optimizations.

    Uses raw PyMongo collection access and projections to bypass Beanie/Pydantic
    overhead for high-density dashboard data.
    """
    ...
    # Use projection to fetch only needed fields and bypass Beanie model instantiation
    user_projection = {"_id": 1, "name": 1, "email": 1, "reward_points": 1}
    employees = await User.get_pymongo_collection().find(
        user_query, user_projection
    ).to_list(length=100000)

    if not employees:
        return []

    # 2. Scope attendance search to these employees and last 5 days
    today_start = ist_now().replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    # Fetch only fields required for the summary
    attendance_projection = {
        "user_id": 1, "check_in": 1, "check_out": 1,
        "location_in": 1, "location_out": 1,
        "address_in": 1, "address_out": 1,
        "remarks": 1
    }
    logs = await Attendance.get_pymongo_collection().find(
        attendance_query, attendance_projection
    ).to_list(length=500000)

    # Map logs by user_id and date for O(1) lookup
    log_map: dict = {}
    # Use IST offset directly for faster date grouping (safe as IST has no DST)
    ist_offset = timedelta(hours=5, minutes=30)
    for log in logs:
        uid = str(log["user_id"])
        date_str = (log["check_in"] + ist_offset).date().isoformat()
        if uid not in log_map:
            log_map[uid] = {}
        if date_str not in log_map[uid]:
            log_map[uid][date_str] = log

    # 3. Assemble final summary
    # Precompute date strings and UTC formats for the last 5 days
    last_5_days_info = []
    for i in range(5):
        day = today_start - timedelta(days=i)
        last_5_days_info.append({
    ...
    for emp in employees:
        ...
        history = []
        for day_info in last_5_days_info:
            date_str = day_info["date_str"]
            record = user_logs.get(date_str)

            entry = {
                "date": day_info["date_utc"],
                "status": "present" if record else "absent",
            }
            if record:
