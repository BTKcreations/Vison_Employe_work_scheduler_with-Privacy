...
async def get_all_attendance_summary(
    ...
    """Get last 5 days attendance summary with optimized database-level filtering and projections."""
    # 1. Optimized User fetching: Push all filters to DB and use projection for minimal data transfer
    ...
    # Bypass Beanie for raw projection performance
    employee_docs = await User.get_pymongo_collection().find(
        user_query,
    ...
    employee_ids = [doc["_id"] for doc in employee_docs]

    # 2. Optimized Attendance fetching
    today_start = ist_now().replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    for log in logs:
        uid = str(log.user_id)
        # IST date string lookup is faster than repeated datetime comparisons
    ...
    # 3. Pre-calculate date metadata to avoid redundant computation in the nested loop
    days_data = []
    # Build list in correct display order (5 days ago -> today) to avoid .reverse() calls
    ...
    for emp in employee_docs:
        ...
        for day_info in days_data:
            record = emp_logs.get(day_info["date_str"])
            entry = {
                "date": day_info["iso"],
                "status": "present" if record else "absent",
            }
            if record:
                entry.update({
                    ...
                    "is_regularized": bool(record.remarks and "Regularized" in record.remarks)
                })
