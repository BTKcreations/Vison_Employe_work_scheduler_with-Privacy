...
async def get_all_attendance_summary(
    ...
    Optimized with database-level filtering and raw projections to bypass Beanie overhead.
    """
    ...
    # Use raw PyMongo projection to bypass Beanie model overhead
    ...
    # Scoped attendance query with raw projection
    att_query = {
    ...
    for log in logs:
        uid = str(log["user_id"])
        # Ensure IST conversion for date grouping
        dt = log["check_in"]
    ...
    for emp in employees:
        ...
        for i in range(5):
            ...
            if record:
                entry["check_in"] = (
                    to_utc_iso(record.get("check_in")) if record.get("check_in") else None
                )
                entry["check_out"] = (
                    to_utc_iso(record.get("check_out")) if record.get("check_out") else None
                )
