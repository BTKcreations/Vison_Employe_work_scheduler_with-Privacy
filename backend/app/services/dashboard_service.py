...
async def get_all_attendance_summary(
    ...
    # Optimization: Use raw PyMongo projection to avoid Beanie model overhead
    ...
    # Pre-calculate date metadata to avoid redundant computation in nested loop
    date_metadata = []
    for i in range(5):
        day = today_start - timedelta(days=i)
    ...
    for emp in employees:
        ...
        for meta in date_metadata:
            ...
            if record:
                entry.update({
                    "check_in": to_utc_iso(record.get("check_in")) if record.get("check_in") else None,
