...
def call_openai_chat_completions(
    prompt: str,
    system_instruction: str = "You are TaskReward AI, an expert workforce intelligence assistant.",
) -> Optional[str]:
...
# -------------------------------------------------------------
async def get_employee_ids_in_scope(
    current_user: User,
...
async def run_task_analysis(
    ...
    for task in active_tasks:
        ...
        if prediction != "On Time":
            insights.append(
                f"Task is at {prediction.lower()} of missing deadline ({round(time_left, 1)} hours remaining)."
            )
...
async def run_performance_analysis(
    ...
    for u in users:
        ...
        avg_completion_time = (
            round(stats["total_hours"] / completed, 1) if completed > 0 else 0.0
        )
        ...
        consistency_score = (
            max(0.0, round(100.0 - late_pct, 1)) if total_days > 0 else 100.0
        )
        ...
        if prod_score >= 80:
            insights.append(
                "High operational productivity. Recommended for milestone bonus incentives."
            )
        elif prod_score < 40 and assigned > 2:
            insights.append(
                "Productivity trajectory shows declining trends. Needs supervisor check-in."
            )

        if burnout_risk == "High":
            insights.append(
                "Burnout warning: working extended hours under high backlog volumes."
            )
        if consistency_score < 70:
            insights.append(
                "Irregular attendance consistency detected. Suggest check-in correction reviews."
            )
    ...
    team_avg_productivity = (
        round(team_total_productivity / valid_employees_count, 1)
        if valid_employees_count > 0
        else 0.0
    )
...
async def run_payroll_analysis(
    ...
    for pr in payroll_records:
        ...
        if pr.net_salary <= 0:
            alerts.append(
                f"Employee {pr.user_name} has a net payout of ₹0.00 for {pr.month}. Verify check-ins or LOP configurations."
            )
    ...
    for ot in overtime_anomalies:
        alerts.append(
            f"Unusual Overtime claim detected for {ot['employee_name']} in {ot['month']}: ₹{ot['overtime_pay']}."
        )
    for sd in suspicious_deductions:
        alerts.append(
            f"Suspiciously high deductions for {sd['employee_name']} in {sd['month']} (₹{sd['deductions']})."
        )
...
async def run_attendance_analysis(
    ...
    for u in users:
        if u.role == UserRole.ADMIN:
            continue
        uid = str(u.id)
        stats = user_attendance.get(
            uid, {"total": 0, "late": 0, "on_time": 0, "absent": 0}
        )
...
async def generate_ai_dashboard_summary(current_user: User) -> Dict[str, Any]:
    ...
    if current_user.role in [UserRole.ADMIN, UserRole.HR_MANAGER]:
        ...
        if len(attendance_intel.get("absentee_warnings", [])) > 0:
            recommendations.append(
                "Initiate HR reviews for personnel showing attendance patterns below threshold."
            )
        ...
        for pred in task_intel.get("task_predictions", []):
            if pred["risk_score"] > 60:
                alerts.append(
                    f"Team Task at risk: '{pred['description']}' assigned to {pred['assigned_to_name']} ({pred['risk_score']}% risk)."
                )

        for o in task_intel.get("overloaded_employees", []):
            alerts.append(
                f"Team Member workload warning: {o['name']} is overloaded with {o['task_count']} active tasks."
            )
        ...
        for pred in task_intel.get("task_predictions", []):
            if pred["risk_score"] > 50 and pred["assigned_to"] == str(current_user.id):
                alerts.append(
                    f"Personal deadline warning: '{pred['description']}' has a {pred['risk_score']}% risk of missing target."
                )
        ...
        for rank in attendance_intel.get("consistency_rankings", []):
            if rank["user_id"] == str(current_user.id):
                if rank["consistency_score"] < 80:
                    alerts.append(
                        f"Consistency warning: your attendance consistency score is {rank['consistency_score']}%. Try logging in on time."
                    )

        # Personal recommendations
        recommendations.append(
            "Ensure task remarks are updated to log completion barriers early."
        )
        recommendations.append(
            "Complete pending regularizations to prevent automated LOP salary deductions."
        )

    # Fallbacks for empty recommendations
    if not recommendations:
        recommendations = [
            "Workforce capacity is operating within standard thresholds. No immediate reallocation required."
        ]
    if not alerts:
        alerts = [
            "All operations are running smoothly. Zero critical warnings flagged."
        ]
    ...
    llm_summary = call_openai_chat_completions(
        prompt,
        "You are a professional HR assistant summarizing operational workforce metrics.",
    )
    if llm_summary:
    ...
    content = {
        ...
        "performance_intelligence": {
            ...
            "burnout_risks": [
                {"name": p["name"], "risk": p["burnout_risk"]}
                for p in perf_intel["employee_performance"]
                if p["burnout_risk"] != "Low"
            ],
        },
    ...
    new_cache = CachedAIInsight(
        user_id=current_user.id, insight_type="dashboard_summary", content=content
    )
...
async def run_ai_copilot_assistant(
    user_message: str, current_user: User
) -> Dict[str, Any]:
    ...
    if "overdue" in message_lc or "delayed" in message_lc or "missed" in message_lc:
        ...
        if tasks:
            task_list = [
                f"'{t.work_description}' assigned to {t.assigned_to_name} (Deadline: {t.deadline.strftime('%d-%b-%Y')})"
                for t in tasks
            ]
            retrieved_facts.append(
                f"Overdue Tasks found in scope:\n- " + "\n- ".join(task_list)
            )
        else:
            retrieved_facts.append("No overdue tasks found in scope.")

    if (
        "overload" in message_lc
        or "capacity" in message_lc
        or "allocation" in message_lc
        or "workload" in message_lc
    ):
        task_intel = await run_task_analysis(scope)
        if task_intel["overloaded_employees"]:
            over = [
                f"{o['name']} ({o['task_count']} active tasks)"
                for o in task_intel["overloaded_employees"]
            ]
            retrieved_facts.append(
            ...
            retrieved_facts.append(
                "No employees are currently overloaded (threshold >= 4 tasks)."
            )
    ...
    if (
        "productivity" in message_lc
        or "best" in message_lc
        or "performance" in message_lc
        or "highest" in message_lc
    ):
        perf_intel = await run_performance_analysis(scope)
        sorted_perf = sorted(
            perf_intel["employee_performance"],
            key=lambda x: x["productivity_score"],
            reverse=True,
        )
        top_perf = [
            f"{p['name']} (Productivity: {p['productivity_score']}%, Burnout Risk: {p['burnout_risk']})"
            for p in sorted_perf[:5]
        ]
        retrieved_facts.append(
            f"Top performing employees by completion rate:\n- " + "\n- ".join(top_perf)
        )
        retrieved_facts.append(
            f"Team average productivity stands at {perf_intel['team_average_productivity']}%"
        )
    ...
    if "attendance" in message_lc or "late" in message_lc or "absent" in message_lc:
        ...
        if not retrieved_facts:
            retrieved_facts.append(
                "Attendance records show no critical late or absence warnings in scope."
            )

    if (
        "payroll" in message_lc
        or "salary" in message_lc
        or "spike" in message_lc
        or "anomaly" in message_lc
    ):
        if current_user.role in [UserRole.ADMIN, UserRole.HR_MANAGER]:
            payroll_intel = await run_payroll_analysis(scope)
            if payroll_intel["alerts"]:
                retrieved_facts.append(
                    "Payroll anomalies / warnings detected:\n- "
                    + "\n- ".join(payroll_intel["alerts"][:5])
                )
            else:
                retrieved_facts.append(
                    "No payroll outliers or variance spikes detected."
                )
        else:
            retrieved_facts.append(
                "Access Denied: payroll intelligence insights are restricted to administrative personnel."
            )
    ...
    facts_text = (
        ...
        if retrieved_facts
        else "No specific operational domain requested. Summarize general status."
    )

    local_answer = ""
    if retrieved_facts:
        local_answer = (
            f"Here are the facts extracted regarding your query:\n\n"
            + "\n\n".join(retrieved_facts)
        )
    else:
        local_answer = (
    ...
    llm_answer = call_openai_chat_completions(
        llm_prompt, "You are a professional corporate assistant copilot for TaskReward."
    )
