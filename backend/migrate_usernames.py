...
async def main():
    ...
    cursor = db.leaves.find(
        {"$or": [{"user_name": {"$exists": False}}, {"user_name": None}]}
    )
        ...
        if user_id:
            ...
            if user:
                ...
                await db.leaves.update_one(
                    {"_id": leaf["_id"]}, {"$set": {"user_name": user_name}}
                )
    ...
    cursor = db.attendance_regularizations.find(
        {"$or": [{"user_name": {"$exists": False}}, {"user_name": None}]}
    )
    updated_reg_count = 0
    async for reg in cursor:
        ...
        if user_id:
            ...
            if user:
                user_name = user.get("name")
                await db.attendance_regularizations.update_one(
                    {"_id": reg["_id"]}, {"$set": {"user_name": user_name}}
                )
                updated_reg_count += 1
