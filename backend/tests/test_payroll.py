...
@pytest.mark.asyncio
async def test_calculate_corporate_payroll_zero_working_days():
    company = Company(name="Test Corp")
    await company.insert()
    user = User(email="test5@example.com", name="Test User 5", role=UserRole.EMPLOYEE, company_id=company.id, hiring_date="2024-05-25", password_hash="mock_hash")
    await user.insert()

    struct = SalaryStructure(user_id=user.id, basic=10000, hra=5000, special_allowance=5000)
    await struct.insert()

    # Let's mock holiday_dates so that all active days are weekends/holidays
    # User joined on May 25th 2024 (Saturday). Let's make May 25-31 all holidays or weekends.
    # May 25 is Saturday, 26 is Sunday, 27-31 are weekdays. We add holidays for 27-31.
    for day in range(27, 32):
        h = Holiday(date=datetime(2024, 5, day, tzinfo=timezone.utc), name="Holiday", company_id=company.id)
        await h.insert()

    payroll = await calculate_corporate_payroll(user, "2024-05")

    assert payroll.total_working_days == 0
    assert payroll.earned_salary == 0.0
